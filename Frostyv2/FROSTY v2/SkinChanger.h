		...
		if (pEntity)
		{
			...
			if (g_Options.Skinchanger.Enabled)
			{
				...
				*pWeapon->FallbackWear() = 0.001f;
				//*pWeapon->FallbackStatTrak() = 3;
				if (g_Options.Skinchanger.StattrakOn)
				{
					*pWeapon->FallbackStatTrak() = 666;
				}
				else
				{
					*pWeapon->FallbackStatTrak() = -1;
				}
				//snprintf(pWeapon->GetCustomName(), 32, "www.insidous.cc");
				if (g_Options.Skinchanger.WeaponTags)
				{
					
				}
				else
				{
					snprintf(pWeapon->GetCustomName(), 32, "insidous.cc" );
				}
